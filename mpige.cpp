...
//     // ------------------------------------------------------------------------------------------
//     void Eliminate(double *matrix, double *rhs, int row, int rows, int columns, int l_rows_start, int size){
...
//             }
//             Eliminate(matrix, rhs, row, rows, columns, l_rows_start, size);
...
//         }
//         // if(rank == size - 1){
//         //     MPI_Send(matrix+rows*columns-1, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
//         // }
//         // if(rank == 0){
//         //     MPI_Recv(matrix+rows*columns-1, 1, MPI_DOUBLE, size-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//         // }
//     }
...
//     //     MPI_Scatter(&matrix[i*size*columns], columns, MPI_DOUBLE, &matrix[(i*size+rank)*columns], columns, MPI_DOUBLE, 0, MPI_COMM_WORLD);
//     //     MPI_Scatter(&rhs[i*size], 1, MPI_DOUBLE, &rhs[i*size+rank], 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
//     // }
//     // MPI_Bcast(matrix, columns, MPI_DOUBLE, 0, MPI_COMM_WORLD);
//     MPI_Bcast(matrix, rows*columns, MPI_DOUBLE, 0, MPI_COMM_WORLD);
//     MPI_Bcast(rhs, rows, MPI_DOUBLE, 0, MPI_COMM_WORLD);

//     // Parallized ForwardElimination
//     MPI::ForwardElimination(matrix, rhs, rows, columns, rank, size);
//     // -------------------------------------------------------------------------------------------
...
//     //---------------------------------------------------------------------------------------------
//     // Uncomment the followed 3 lines if you would like to run unparallized BackwardSubstitution
//     if(rank==0){
...
//     double *sub_matrix = new double[rows * num_rows];
//     double *sub_rhs = new double[num_rows];

//     for(int i=0; i< num_rows;i++){
//         MPI_Scatter(&matrix[i * rows *size], rows, MPI_DOUBLE, &sub_matrix[i*rows], rows, MPI_DOUBLE, 0, MPI_COMM_WORLD);
//         MPI_Scatter(&rhs[i*size], 1, MPI_DOUBLE, &sub_rhs[i], 1, MPI_DOUBLE , 0, MPI_COMM_WORLD);
//     }



//     double *temp_row = new double[rows];
//     double temp_rhs;
...
//         if(rank == which_rank){
//             pivot = sub_matrix[local_row*rows + i];

//             for(int j = i + 1; j < rows; j++){
//                 sub_matrix[local_row * rows + j] /= pivot;
//             }
//             sub_rhs[local_row] /= pivot;

//             sub_matrix[local_row * rows + i] = 1;

//             memcpy(temp_row, &sub_matrix[local_row * rows], rows * sizeof(double));
//             temp_rhs = sub_rhs[local_row];

//             MPI_Bcast(temp_row, rows, MPI_DOUBLE, which_rank, MPI_COMM_WORLD);
//             MPI_Bcast(&temp_rhs,1,MPI_DOUBLE, which_rank, MPI_COMM_WORLD);
//             for(int j= local_row + 1; j < num_rows; j++){
//                 scale= sub_matrix[j * rows + i];

//                 for( int k = i + 1; k<rows; k++){
//                     sub_matrix[j*rows+k]-=scale * temp_row[k];
//                 }
//                 sub_rhs[j]-=scale*temp_rhs;

//                 sub_matrix[j*rows+i]=0;
//             } 
//         }
//         else{
//             MPI_Bcast(temp_row, rows, MPI_DOUBLE, which_rank, MPI_COMM_WORLD);
...
//     for(int i=0; i< num_rows;i++){
//         MPI_Gather(&sub_matrix[i*rows], rows, MPI_DOUBLE, &matrix[i*size*rows], rows, MPI_DOUBLE, 0, MPI_COMM_WORLD);
//         MPI_Gather(&sub_rhs[i], 1, MPI_DOUBLE, &rhs[i*size], 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
//     }
